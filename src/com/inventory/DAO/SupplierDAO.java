...
public class SupplierDAO {
    ...
    public Map<String, Integer> getSuppliesBySupplier() {
        Map<String, Integer> supplyData = new HashMap<>();

        try {
            String query = "SELECT p.fullname AS supplier_name, SUM(pi.quantity) AS supply_count " +
                    "FROM purchaseinfo pi " +
                    "JOIN suppliers p ON pi.suppliercode = p.suppliercode " +
                    "GROUP BY p.fullname";
            PreparedStatement stmt = conn.prepareStatement(query);
            ResultSet rs = stmt.executeQuery();

            while (rs.next()) {
                supplyData.put(rs.getString("supplier_name"), rs.getInt("supply_count"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
