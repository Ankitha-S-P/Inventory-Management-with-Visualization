...
public class CustomerDAO {
    ...
    public Map<String, Integer> getCustomerPurchases() {
        Map<String, Integer> purchaseData = new HashMap<>();

        try {
            // Query to get the customer name and count of purchases
            String query = "SELECT c.fullname AS customer_name, COUNT(*) AS purchase_count " +
                    "FROM salesinfo si " +
                    "JOIN customers c ON si.customercode = c.customercode " +
                    "GROUP BY c.fullname";
            ...
            while (rs.next()) {
                purchaseData.put(rs.getString("customer_name"), rs.getInt("purchase_count"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
