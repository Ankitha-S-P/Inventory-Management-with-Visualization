...
public class ProductDAO {
    ...
    public Map<String, Double> getRevenueShareByProducts() {
        Map<String, Double> revenueData = new HashMap<>();

        try {
            // Query to calculate total revenue by product
            String query = "SELECT p.productname, SUM(si.revenue) AS total_revenue " +
                    "FROM salesinfo si " +
            ...
            while (rs.next()) {
                revenueData.put(rs.getString("productname"), rs.getDouble("total_revenue"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Map<String, Integer> getStockLevels() {
        ...
        try {
            // Query to retrieve the product name and current stock quantity
            String query = "SELECT p.productname, cs.quantity " +
                    "FROM currentstock cs " +
                    "JOIN products p ON cs.productcode = p.productcode";
            PreparedStatement stmt = conn.prepareStatement(query);
            ResultSet rs = stmt.executeQuery();

            while (rs.next()) {
                stockData.put(rs.getString("productname"), rs.getInt("quantity"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
